...
- When a proposed or implemented deepening would retire existing tests, read `test-quality`'s [deletion gate](../../test-quality/SKILL.md#deletion-gate) before recommending their disposition. Apply that gate in design assessments as well as implementation. Once interface-level tests cover the same behavior, identify tests of the merged shallow modules as retirement candidates. Aim to replace coverage rather than layer it. Any survivor that fails on an implementation-only change was pinned to the implementation: rewrite it against the interface when test edits are in scope, or identify it for removal through that gate.
