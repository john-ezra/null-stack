...
3. Resolve the requested destination and completion policy. A linked issue is not permission to write to its tracker. With permission, publish the note to that issue, then apply only the completion transition authorized by project policy or the request when its conditions are met. Use `linear-cli` for Linear comments and explicit state updates. Without a tracker or write permission, return the note in chat; do not create records to close out a standalone PR. Effort completion and archival remain separate project decisions.
