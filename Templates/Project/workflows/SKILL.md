...
| Picked-up requirements may need to change | `spec` owns the After pickup scope decision and amendment, replacement, or follow-on record | Keep the original contract unless the user approves a change; publish only with the permissions in the corresponding event below. |
...
| Spec for one unit of work | Linear issue description | Follow `spec`'s source-provenance rule for an explicit body reference or a confirmed project-to-intent association. |
...
| An intent is ready to slice or re-slice | Use `spec` to present the breakdown, then the completed specs for approval. With publication permission, put approved specs in issue descriptions at `Todo` under the project; publish coarse placeholders at `Backlog`. Apply `spec`'s source-provenance rule before omitting `Source:`; confirm the project's overview link resolves to the intent. Create confirmed blocking relations after publishing blockers. | Every approved frontier slice has one issue at `Todo`, its source is explicit or confirmed under `spec`, every remaining placeholder is at `Backlog`, and the native relations express the blocking order. |
...
| A picked-up scope decision has approved records | Follow `spec`'s After pickup procedure. With tracker-write permission, append an amendment to the existing issue description, or publish replacement/follow-on specs as new issues at `Todo`. Keep an amended issue `In Progress`; supersede an old issue through the terminal event below only for a replacement. Revise affected relations within the authorized scope. | The original requirements and the approved change's source and approval remain identifiable under `spec`. Published records and relations match that decision, or the proposed records stay in chat with no tracker writes. Approval alone does not publish or start the new work. |
...
An issue that outgrows one session goes through `intent` and the finite-effort event. Keep the original issue as its first spec only if its approved contract still fits; changes to a picked-up contract follow `spec`'s After pickup procedure. A project that collapses to one slice keeps its issue; on the user's word, detach that issue and cancel the redundant project. Preserve the intent file and its history unless the user authorizes a separate move. Either resizing path is done when the surviving issue, project association, and relations match the approved scope.

Each artifact freezes on its own clock. The intent settles once shaped and approved; later changes follow `intent`'s revision rules. The spec and plan freeze at pickup. For a proposed change to frozen requirements, read `spec` and follow its After pickup procedure for the user's scope decision and preserved approval history. The event above places those records only with separate tracker-write permission. The PR becomes the permanent record of the shipped change at merge; design-decision records follow the shared decision-record guidance.
