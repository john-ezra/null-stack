...
1. Choose which example policies below the project wants to keep. Replace the artifact homes, sizing rule, state mapping, and completion procedure here when adopting different policies. Keep artifact content and format in the owning skills. Adaptation is done when the retained rules describe the project's actual process rather than choices left to the agent.
...
| A PR is requested | `pr` owns its title, body, decisions, and verification evidence | Draft for review or publish to the repository's PR host as authorized; finish by the skill's completion test. |
...
These are choices for this workflow, not requirements of the skills or project starter.
...
| Intent for a finite effort | `agent-docs/intents/<slug>.md`, landed on the default branch | The Linear project overview links to the file on that branch; it does not carry a second copy. |
...
| Implementation and verification evidence | Repository change and PR | The PR records decisions and observed proof using `pr`; the issue receives a close-out comment after merge. |
