...
- Linear: follow the [Linear write policy](agent-docs/LINEAR.md#write-permission) for tracker changes. Without a recorded policy, change records only when explicitly asked.
...
---
1. Copy this file as the project's `AGENTS.md`. Copy the companion templates into `agent-docs/`: [PROJECT-OUTLINE.md](PROJECT-OUTLINE.md), [DEVELOPMENT.md](DEVELOPMENT.md), [GIT.md](GIT.md), [CODE-STYLE.md](CODE-STYLE.md), and [LINEAR.md](LINEAR.md). Copy [RESEARCH.md](RESEARCH.md) as `agent-docs/research/README.md` and [DESIGN-DECISIONS.md](DESIGN-DECISIONS.md) as `agent-docs/design-decisions/README.md`, or remove their rows if the project does not use them. If Linear is not used, omit its template and remove its row, rule, and other references. Source links in these adoption steps resolve beside this template; finished-project links elsewhere resolve from each file's destination. For layout and assembly choices, read [Project Starter](PROJECT-STARTER.md).
...
5. Keep lifecycle policy in `agent-docs/DEVELOPMENT.md` and only its pointer here. Author task workflows separately from [WORKFLOW.md](WORKFLOW.md), and add overview links only after their files exist. Copying a template adopts neither a lifecycle nor a workflow, and reading a guide authorizes no execution or publication.
