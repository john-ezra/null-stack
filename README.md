...
A collection of agent skills and authoring templates. Use a skill on its own, or assemble project guidance and workflows to suit your work.
...
| [templates/PROJECT-STARTER.md](templates/PROJECT-STARTER.md) | Layout and assembly guide for an adaptable private workspace with the project's code as a Git submodule; maps templates to their destinations |
...
| [templates/RESEARCH.md](templates/RESEARCH.md) | Guide for saving and consulting research reports; becomes `agent-docs/research/README.md` |
| [templates/DESIGN-DECISIONS.md](templates/DESIGN-DECISIONS.md) | Guide for selecting, approving, writing, and revising lasting decisions; becomes `agent-docs/design-decisions/README.md` |
...
Use [Project Starter](templates/PROJECT-STARTER.md) for the suggested workspace layout and source-to-destination table. It explains how to assemble the workspace from templates, which files to create yourself, and what to omit when you do not use Linear or workflows. It is a reference to adapt, not a filled project to copy.

Follow the annotated templates to choose and write your project's guidance. For an existing project, reconcile its current instructions rather than overwrite them. Preserve the toolkit's [license notice](#license) without replacing either repository's license.

### Author a project AGENTS.md

Start with [templates/AGENTS.md](templates/AGENTS.md) and follow its "How to use this template" steps for the overview and companion guides. The templates stay flat, with authoring instructions beside the content they explain. Each names its destination; finished-project links resolve from that destination, while source links in the adoption steps lead to neighbouring templates.

Keep authoring notes and unused examples out of the finished project. Retain the research and design-decision guides' rules and record format as project guidance, adapting them only where the project differs. Ordinary work with these guides needs no network access, toolkit checkout, installed skills, or host-specific resolver.

### Author a project workflow

For a recurring task, use [templates/WORKFLOW.md](templates/WORKFLOW.md). It holds the authoring steps and one generic fenced block, not shipped examples, installed commands, or preselected recipes.
...
Check adopted or adapted guidance outside the toolkit checkout. Every retained local link and anchor must resolve from the consuming project, and required skills must be available for any integrations or workflows it adopts. Authoring instructions belong in the templates, not the finished project; keep formats for documents created during ordinary work.
